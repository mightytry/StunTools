...
namespace StunClient
{
    internal class Hosts
    {
        ...
        public static async Task<IPEndPoint?> SolveDns(string name, uint port)
        {
            try
            {
                var x = await Dns.GetHostAddressesAsync(name);

                return new IPEndPoint(x.Where(x => x.AddressFamily == AddressFamily.InterNetwork).First(), (int)port);
            }
            catch
            {
                return null;
        ...
        public static Dictionary<string, uint> List = new()
        {
